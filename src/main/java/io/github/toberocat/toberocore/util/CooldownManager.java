...
public class CooldownManager {
    private static final @NotNull SimpleDateFormat defaultFormat = new SimpleDateFormat("m'm'ss's'");
    ...
    public static @NotNull CooldownManager createManager(@NotNull TimeUnit timeUnit, long duration) {
        return createManager(defaultFormat, timeUnit, duration);
    }

    public static @NotNull CooldownManager createManager(@NotNull SimpleDateFormat format, @NotNull TimeUnit timeUnit, long duration) {
        return new CooldownManager(format, timeUnit.toSeconds(duration) * 20);
    }

    public void activateCooldown(@NotNull JavaPlugin plugin, @NotNull UUID player) {
        cooldowns.put(player, System.currentTimeMillis() + cooldownMs);
        Bukkit.getScheduler().runTaskLater(plugin, () -> cooldowns.remove(player), cooldownTicks);
    }
    ...
    public long getLeftCooldown(@NotNull UUID player) {
        return untilInCooldown(player).orElse(System.currentTimeMillis()) - System.currentTimeMillis();
    }
    ...
    public void runCooldown(@NotNull JavaPlugin plugin, @NotNull UUID player) throws CooldownException {
        if (hasCooldown(player)) throw new CooldownException(this, player);
        activateCooldown(plugin, player);
    }
